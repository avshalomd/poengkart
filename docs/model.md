...
same programme at the same school moves by a standard deviation of 6.4 points
from one year to the next (5 538 consecutive-year pairs; only half of all
moves are within ±3).
...
similar ones around it instead of being trusted on its own —429 of the 2 021
series have exactly one year.

**Level** (on the 8 414 cells that carry a number):
...
backtest — it barely mattered, 4 years won by 0.017 RMSE over no decay). A
...
Fitted variance components (points): school 3.1, programme 3.3, series 2.7,
...
Trøndelag, which do not state their round, are no problem. A year published
from another round inside a county's series gets a fixed offset per category
from the round bridge below, so the random walk does not learn the dip as a
market event; Vestland 2023 was that year until its 1. inntak file was
...
×1.13 below 25 points, ×1.01 from 25 to 40, ×0.96 from 40 to 45, ×0.64 at 45 and above (fitted on
...
band's 80% coverage from 97.0% to 80.6% and the bottom band's from 70.0% to
77.5%, and nothing else.
...
**Level, held-out 2025–26** (2 297 cells that got a number):

| history | n | model RMSE | "last year's figure" RMSE | programme-county mean RMSE | within ±3 |
|---|---|---|---|---|---|
| 0 years | 167 | 8.6 | — | 10.2 | 30% |
| 1 year | 314 | 6.2 | 7.4 | 7.0 | 46% |
| 2–3 years | 461 | 5.9 | 6.8 | 6.5 | 42% |
| 4+ years | 1355 | 5.1 | 6.2 | 6.0 | 51% |
...
and 5.2 with four or more, against the model's 5.9 and 5.1 — most of the
...
(logit π′ = 0.170 + 0.587 logit π). Scored on the eight counties with a
fill state, Møre og Romsdal's proxy labels included and Telemark excluded:
held-out Brier 0.157 against 0.208 for the
...
held-out Brier goes from 0.155 to 0.155 and the Platt slope from 0.587 to
0.541; on the county's own 223 held-out cells the proxy-labelled hurdle
scores 0.147 against 0.186 for its base rate
(`meta.halflife_search.proxy_label_experiment`).
Telemark (17 September 2026) has no fill state at all — the county gives a
number for every offered programme — and sits in `FILL_BLIND`: it trains
the level model, its fill probability is fixed at 1, and it is outside every
fill score, so the fill figures below are over the eight other counties.
...
Below 70% the forecast is within 4.9 points of the outcome in every bin,
...
up it is cautious — a stated 75% came true 82% of the time, the largest gap
...
Decomposing the raw mean that the map colours by, over the 192 schools whose
α rests on five or more fitted cells: the school's own effect explains 43%
of the variance between schools, the programme mix 22%, the county's level
...
mean, schools move 3.0 places on average and at most 25 — a good part of a
...
`meta.outliers` (|z| ≥ 3: 75 of 8 414 cells, 36 of them in Vestland, the
