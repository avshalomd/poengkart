...
const ordEn = r => `${r}${r === '1' ? 'st' : r === '2' ? 'nd' : r === '3' ? 'rd' : 'th'}`;
const T = {
...
const t = (k, ...a) => {
  ...
  const e = T[k], v = e[lang] !== undefined ? e[lang] : e.no;
...
const REPO = 'https://github.com/avshalomd/poengkart';
const REPO_LINK = `<a href="${REPO}" target="_blank" rel="noopener">GitHub ↗</a>`;
...
const round1 = v => { const h = Math.round(v * 100), sg = h < 0 ? -1 : 1; return sg * Math.round(Math.abs(h) / 10) / 10; };
...
const isPoints = v => typeof v === 'number' && v > 0;
...
function openMix(programs, year) {
...
const fmtNum = v => typeof v === 'number' ? round1(v).toFixed(1).replace('.', ',') : v;
const esc = s => String(s).replace(/[&<>"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));

let DATA = null, map = null, markerLayer = null;
let MODEL = null;         // data/model.json — forecasts; optional
let myPoints = null;      // the reader's own points, or null
let showOld = false;      // show programme areas with no figures lately
let allLevels = false;    // show Vg2 and up as well; Vg1 alone is the default (levelScope)
let HOME = null;          // bounds of the whole dataset, the map's home view
let mapCat = 'all';
let mapFylke = 'all';
...
let chart = { prog: null };
const chartMode = () => chart.prog ? 'prog' : (mapCat !== 'all' ? 'cat' : 'all');
...
const fmt = v => {
  const out = lang === 'no' ? fmtNum(v) : String(fmtNum(v)).replace(',', '.');
  return typeof out === 'string' ? out.replace(/^-/, '\u2212') : out;
};
const X_ICON = '<svg class="xi" viewBox="0 0 24 24" aria-hidden="true"><path d="M18 6 6 18M6 6l12 12"/></svg>';
const BUG_ICON = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">'
...
const progName = p => (lang === 'en' && p.program_en) || p.program;
...
const progId = p => `${p.program.toLowerCase()}|${p.level}`;
...
const visibleIn = progs => { const pp = partitionPrograms(progs); return pp.regular.length + pp.orphans.length; };
const visibleCount = s => visibleIn(shownPrograms(s));
...
let _newestByFylke = null;
function countyNewest(f) {
  if (!_newestByFylke) {
    _newestByFylke = {};
    DATA.schools.forEach(s => s.programs.forEach(p => Object.keys(p.values).forEach(y => {
      _newestByFylke[s.fylke] = Math.max(_newestByFylke[s.fylke] || 0, +y);
    })));
  }
  return _newestByFylke[f] || 0;
}
const isRecent = (p, f) => Object.keys(p.values).some(y => +y >= countyNewest(f) - 1);
...
const isVg1 = p => p.level === 'Vg1';
function levelScope(progs) {
  if (allLevels) return progs;
  const vg1 = progs.filter(isVg1);
  return vg1.length ? vg1 : progs;
}
function shownPrograms(s) {
  const pool = levelScope(s.programs);
  if (showOld) return pool;
...
const staleBefore = () => +DATA.years[DATA.years.length - 1] - 1;
...
const OPEN_RULE = new Set(['Møre og Romsdal']);
// counties that do not publish poenggrenser (docs/data-notes.md)
const MISSING_COUNTIES = ['Agder', 'Finnmark', 'Nordland', 'Telemark', 'Troms', 'Vestfold', 'Østfold'];
...
const zeroLabel = (zeroN, openN) => openN ? t('zeroMix', zeroN, openN) : t('noPoints');
...
const BANDS = { likely: 0.70, possible: 0.35 };
const ZQ_GRID = Array.from({ length: 41 }, (_, i) => i / 40);
function errCdf(z) {
  const q = MODEL && MODEL.meta && MODEL.meta.error_quantiles;
...
const chanceOf = (pr, x) => (1 - pr.pi) + pr.pi * errCdf((x - pr.m) / pr.s);
const bucketOf = c => c >= BANDS.likely ? 'likely' : c >= BANDS.possible ? 'possible' : 'unlikely';
const bucketColor = b => cssVar(b === 'likely' ? '--good' : b === 'possible' ? '--dot-possible' : '--dot-unlikely');
...
const pctS = c => lang === 'no' ? `${pct(c)} %` : `${pct(c)}%`;
const chanceMode = () => myPoints !== null && !!MODEL;
// the model's raw entry for one programme row, or null; keys match tools/model.py
function modelEntry(s, p) {
  const e = MODEL && MODEL.schools && MODEL.schools[`${s.fylke}|${s.name}`];
...
function finalRoundBridge(s) {
  const rb = MODEL && MODEL.meta && MODEL.meta.round_bridge;
  if (!rb || !s.round) return null;
  const b = Object.values(rb).find(b => b.fylke === s.fylke && b.from_round === s.round);
  return b && +b.to_round > +s.round ? b : null;
}
function chanceFinal(pr, x, cat, b) {
...
const isChosen = (s, p) => choices.some(c => c.f === s.fylke && c.s === s.name && c.k === progKeyMap(s).get(p));
let choicesNote = null;   // why the last add was refused; rendered once
let pickNoteTimer = null;
function pickNote(msg) {
  ...
  clearTimeout(pickNoteTimer);
  pickNoteTimer = setTimeout(() => el.classList.remove('show'), 4500);
}
function toggleChoice(s, p) {
  const k = progKeyMap(s).get(p);
  const i = choices.findIndex(c => c.f === s.fylke && c.s === s.name && c.k === k);
  choicesNote = null;
  if (i >= 0) choices.splice(i, 1);
    ...
    const items = choices.map(resolveChoice).filter(Boolean);
    const refuse = key => {
      choicesNote = t(key);
      renderChoices();
      ...
      const note = document.querySelector('#choices .vnote'), card = document.getElementById('panel');
      ...
      if (!onScreen) pickNote(choicesNote);
    };
    ...
    choices.push({ f: s.fylke, s: s.name, k });
  }
  try { localStorage.setItem('pk-choices', JSON.stringify(choices)); } catch (e) {}
  renderChoices();
...
function resolveChoice(c) {
  if (!okChoice(c) || !DATA) return null;
  const s = DATA.schools.find(x => x.fylke === c.f && x.name === c.s);
  if (!s) return null;
...
function renderChoices() {
  ...
  const before = JSON.stringify(choices), seen = new Set();
  const kept = choices.filter(c => resolveChoice(c))
  ...
  items.forEach(({ s, p }, i) => {
    ...
    if (chanceMode()) {
      const pr = predFor(s, p);
      if (pr) {
        const c = chanceOf(pr, myPoints), b = bucketOf(c);
  ...
  if (chanceMode() && n) {
    const bar = [['likely', L], ['possible', R], ['unlikely', U]]
      .map(([b, k]) => `<span class="b-${b}" style="width:${100 * k / n}%"></span>`).join('');
  ...
  box.innerHTML = `<div class="h"><span>${esc(t('choicesHead', items.length))}</span>` +
    ...
    (choicesNote ? `<div class="vnote">⚠ ${esc(choicesNote)}</div>` : '') + sum;
  box.querySelectorAll('.who').forEach(b => b.onclick = () => {
    const { s } = items[+b.dataset.i]; openSide(s);
    if (s.lat && map && view === 'map') map.setView([s.lat, s.lon], Math.max(map.getZoom(), 11));
  });
  ...
  clr.onclick = () => {
    if (!clr.dataset.armed) {
      ...
      clearTimeout(renderChoices.armTimer);
      renderChoices.armTimer = setTimeout(() => {
...
const PTS_OK = /^\d{1,2}([.,]\d{1,2})?$/;
const PTS_TYPING = /^\d{0,2}([.,]\d{0,2})?$/;
let ptsBad = false;
...
function onPoints(v) {
  const inp = document.getElementById('my-points');
  const r = parsePoints(v);
  myPoints = r.pts;
  ptsBad = r.bad;
  if (v === '') inp.value = '';
  try {
    if (myPoints === null) localStorage.removeItem('pk-points');
    else localStorage.setItem('pk-points', String(myPoints));
  } catch (e) {}
...
function renderPointsField() {
  ...
  if (myPoints !== null && document.activeElement !== inp) inp.value = fmt(myPoints);
  const bad = ptsBad && inp.value.trim() !== '';
  inp.classList.toggle('on', myPoints !== null);
...
/* ================= map ================= */
let labelMarkers = () => {};
let mapFocusPending = 0;   // when Enter zoomed into a cluster, for the focus to follow
...
const TIP_OFF = { top: [0, -10], bottom: [0, 10], left: [-10, 0], right: [10, 0] };
const tipOpts = (dir, dx = 0) => ({ className: 'tip', direction: dir, opacity: 1, offset: [TIP_OFF[dir][0] + dx, TIP_OFF[dir][1]] });
function aimTip(m, html, wrapTo = 0) {
  if (!map || !m.getTooltip()) return;
  ...
  wrap(te);
  const w = te.offsetWidth, h = te.offsetHeight, W = map.getSize().x, H = map.getSize().y;
  const pt = map.latLngToContainerPoint(m.getLatLng());
  const mr = map.getContainer().getBoundingClientRect(), pr = document.getElementById('panel').getBoundingClientRect();
  const pn = view === 'map' && pr.width ? { l: pr.left - mr.left, t: pr.top - mr.top, r: pr.right - mr.left, b: pr.bottom - mr.top } : null;
...
function lensNoFigure(s) {
  const scope = levelScope(s.programs).filter(p => p.category === mapCat);
...
let tileLayer = null, miniMap = null, miniMapRO = null;
let sideOpener = null;      // the marker or cluster that opened the panel, for Escape to return to
const markerOf = new WeakMap();   // a marker's element → its layer, so closeSide can reach the tooltip
...
const CARTO_KEY = 'cb1_2bsv_1_23dae695a38f70885d3b4f7b';
...
function drawMarkers() {
  ...
  for (const s of visibleSchools()) {
    ...
    const ch = chanceMode() ? schoolChance(s, mapCat, myPoints) : null;
    ...
    const m = L.circleMarker([s.lat, s.lon],
      { ...style, pkSchool: s, pkBucket: ch ? bucketOf(ch.best) : 'none' }).addTo(markerLayer);
    let line;
    if (chanceMode()) {
      line = (mapCat === 'all' ? '' : `${CATS[mapCat][lang]}: `) + (ch
        ? t('tipChance', fmt(myPoints), ch.likely, ch.possible, ch.unlikely, ch.n, ch.year)
      ...
      if (st8.kind === 'points') {
        ...
        const sy = yearSpan(s);
        line = t('tipNoData', sy ? sy.split('–').pop() : DATA.years[DATA.years.length - 1]);
      }
    } else {
      line = st8.kind === 'points'
             ? `${CATS[mapCat][lang]}: ` + t('tipMedian', fmt(st8.v), st8.filled, st8.total)
               + (st8.mostlyOpen ? `<br><span class="warn">⚠ ${t('tipMostlyOpen')}</span>` : '')
           : st8.kind === 'open' ? `${CATS[mapCat][lang]}: ${t('allIn').toLowerCase()} (${st8.year})`
           : st8.kind === 'zero' ? `${CATS[mapCat][lang]}: ${zeroLabel(st8.zeroN, st8.openN).toLowerCase()} (${st8.year})`
           : st8.kind === 'stale' ? `${CATS[mapCat][lang]}: ` + t('tipStale', st8.year)
           : `${CATS[mapCat][lang]}: ${lensNoFigure(s)}`;
    }
  ...
  const labelClusters = () => {
    ...
    for (const l of (fg ? fg.getLayers() : [])) {
      ...
      el.addEventListener('keydown', ev => {
        ...
        ev.preventDefault();
        mapFocusPending = Date.now();    // the zoom removes this element; see below
        if (l._bounds && map.getBoundsZoom(l._bounds) > map.getZoom()) l.zoomToBounds({ padding: [30, 30] });
        else l.spiderfy();
      });
    }
  };
  ...
  const labelPoints = labelMarkers;
  labelMarkers = () => {
    ...
    if (mapFocusPending && Date.now() - mapFocusPending < 3000
        && (!document.activeElement || document.activeElement === document.body)) {
      mapFocusPending = 0;
      (mapKeyed().find(e => e.dataset.pkRove === '0') || map.getContainer()).focus({ preventScroll: true });
    }
  };
  markerLayer.on('animationend', () => { labelMarkers(); legendZoomHint(); });
  labelMarkers();
}
...
function onMapFylke(v) {
  ...
  if (mapCat !== 'all' && !DATA.schools.some(s =>
      (v === 'all' || s.fylke === v) && shownPrograms(s).some(p => p.category === mapCat))) {
    mapCat = 'all';
  }
  ...
  if (current && !visibleSchools().includes(current)) closeSide(true);
  ...
  if (view === 'list' || !map.getContainer().clientWidth) { refitPending = true; return; }
  const pts = visibleSchools().filter(s => s.lat).map(s => [s.lat, s.lon]);
  if (pts.length) {
    const box = L.latLngBounds(pts).pad(0.08);
    if (matchMedia('(prefers-reduced-motion: reduce)').matches) map.fitBounds(box, framePad());
    else map.flyToBounds(box, { ...framePad(), duration: .6 });
  }
}
...
function visibleSchools() {
  return DATA.schools.filter(s =>
    ...
    (mapCat === 'all' || shownPrograms(s).some(p => p.category === mapCat)));
}

function setLens(v) {
  mapCat = v;
  document.getElementById('map-cat').value = v;
  // a selected row survives any lens that still contains it
  if (chart.prog && v !== 'all' && chart.prog.category !== v) chart.prog = null;
  if (current && !visibleSchools().includes(current)) closeSide(true);
  drawMarkers(); renderLegend(); renderCatNote(); renderPanelSum();
  renderSide();
  syncUrl(true);
}
function onMapCat(v) { setLens(v); }
...
function setLevels(v) {
  allLevels = !!v;
  try { localStorage.setItem('pk-alllevels', allLevels ? '1' : '0'); } catch (e) {}
  if (chart.prog && current && !levelScope(current.programs).includes(chart.prog)) chart.prog = null;
  if (mapCat !== 'all' && (current ? !shownPrograms(current).some(p => p.category === mapCat)
                                   : !visibleSchools().length)) mapCat = 'all';
...
const laterPublished = () => DATA.schools.some(s => (mapFylke === 'all' || s.fylke === mapFylke)
...
function renderPanelSum() {
  if (!DATA) return;
  const parts = [];
  if (!document.getElementById('fylke-field').hidden)
    parts.push([t('fylkeLabel'), mapFylke === 'all' ? t('allFylker') : mapFylke]);
  parts.push([t('catLabel'), mapCat === 'all' ? t('allCats') : CATS[mapCat][lang]]);
  // the scope only when it is not the default: the folded line has to say
  // what the checkbox it hides is set to
  if (allLevels && laterPublished()) parts.push([t('levelsSumLabel'), t('levelsChipAll')]);
  document.getElementById('panel-sum-t').innerHTML = parts.map(([l, v], i) =>
    `<span class="ln"><span class="vh">${i ? ', ' : ''}${esc(l)}: </span>${esc(v)}</span>`).join('');
}
const panelFolds = () => view === 'map' && matchMedia('(max-width: 560px), (max-height: 480px)').matches;
function foldPanel(fold) {
...
function renderPanel() {
  ...
  document.getElementById('tagline').replaceChildren(Object.assign(document.createElement('span'), {
    textContent: t('tagline', DATA.schools.length, DATA.counties.length,
  ...
  fsel.innerHTML = `<option value="all">${t('allFylker')}</option>` +
    counties.map(c => {
      const n = DATA.schools.filter(s => s.fylke === c.fylke && s.lat).length;
...
function renderCatNote() {
  ...
  el.textContent = mapCat === 'all' ? '' :
    t('catNote', DATA.schools.filter(s =>
      (mapFylke === 'all' || s.fylke === mapFylke)
      && shownPrograms(s).some(p => p.category === mapCat)).length);
}
function liftMapControls() {
...
function legendZoomHint() {
  const el = document.getElementById('legend-zoom');
  if (!el) return;
  const fg = markerLayer && markerLayer._featureGroup;
...
function forecastYears(schools) {
  const ys = [...new Set(schools.map(s => MODEL?.schools?.[`${s.fylke}|${s.name}`]?.year).filter(Boolean))].sort();
  return t('yearsOr', ys);
}
// the dots are Vg1 unless the reader asked for the later years and the
// counties on screen publish any: the legend says which
function levelChip() {
  const on = allLevels && visibleSchools().some(s => s.programs.some(p => !isVg1(p)));
...
function renderLegend() {
  ...
  document.getElementById('legend-title').innerHTML =
    (chanceMode() ? esc(t('legendChance', fmt(myPoints), forecastYears(visibleSchools())))
...
const schoolPart = s => 's=' + encodeURIComponent(s.fylke) + '/' + encodeURIComponent(s.name);
function buildHash(s) {
  const parts = [];
  if (s) parts.push(schoolPart(s));
  if (mapFylke !== 'all') parts.push('f=' + encodeURIComponent(mapFylke));
  if (mapCat !== 'all') parts.push('c=' + encodeURIComponent(mapCat));
  if (allLevels) parts.push('l=all');
  return parts.length ? '#' + parts.join('&') : location.pathname + location.search;
}
const schoolHash = s => buildHash(s);
function hashParts() {
  const out = {};
...
function schoolFromUrl() {
  ...
  try {
    ...
    const inFylke = DATA.schools.filter(x => x.fylke === fy);
...
function applyUrlFilters(boot) {
  ...
  let lv = 'l' in p ? p.l === 'all' : boot ? allLevels : false;
  try {
    if (p.f) fy = decodeURIComponent(p.f);
    if (p.c) cat = decodeURIComponent(p.c);
  } catch (e) { return false; }
  if (!DATA.schools.some(s => s.fylke === fy)) fy = 'all';        // a county we do not carry
  if (cat !== 'all' && !CATS[cat]) cat = 'all';
  // a lens with nothing at Vg1 (påbygging) is a link to the later years
  if (cat !== 'all' && !lv && !DATA.schools.some(s => (fy === 'all' || s.fylke === fy)
        && s.programs.some(q => isVg1(q) && q.category === cat))) lv = true;
  if (fy === mapFylke && cat === mapCat && lv === allLevels) return false;
  mapFylke = fy; mapCat = cat; allLevels = lv;
  const fs = document.getElementById('map-fylke'), cs = document.getElementById('map-cat');
  if (fs) fs.value = fy;
  renderPanel();
  if (cs) cs.value = cat;
...
const LIST_LAYOUT = { n: [1068, 1582], lg: [1168, 1734], xl: [1268, 1905] };
function listLayout() {
  const [split, wide] = LIST_LAYOUT[document.documentElement.dataset.font] || LIST_LAYOUT.n;
  const lv = view !== 'list' ? '' : innerWidth >= wide ? 'wide' : innerWidth >= split ? 'split' : 'thin';
...
const sheetFull = () => phoneSheet() || (view === 'list' && listLayout() === 'thin');
function sideTrap(on) {
  ...
  const covered = on && view === 'list' && sheetFull();
...
function widenFor(s) {
  ...
  let moved = false;
  if (mapFylke !== 'all' && s.fylke !== mapFylke) { mapFylke = 'all'; moved = true; }
  if (mapCat !== 'all' && !shownPrograms(s).some(p => p.category === mapCat)) { mapCat = 'all'; moved = true; }
  if (!moved) return false;
  const fy = document.getElementById('map-fylke'), ct = document.getElementById('map-cat');
  if (fy) fy.value = mapFylke;
  renderPanel();                       // the category list depends on the county
  if (ct) ct.value = mapCat;
...
const mapKeyed = () => [...document.querySelectorAll('#map [role="button"][data-pk-keyed]')];
...
function buildMiniMap(s) {
  ...
  if (miniMapRO) miniMapRO.disconnect();
  miniMapRO = new ResizeObserver(() => miniMap && miniMap.invalidateSize());
  miniMapRO.observe(document.getElementById('s-minimap'));
}
function openSide(s) {
  const ae = document.activeElement;
  sideOpener = ae && document.getElementById('map')?.contains(ae) ? ae : null;
  ...
  if (!document.getElementById('side').classList.contains('open')) {
    sideReturn = ae?.closest?.('#choices .who') ? { who: ae.getAttribute('aria-label') }
      : ae?.closest?.('#searchov') ? { id: 'searchov-btn' } : null;
  }
  widenFor(s);
  current = s;
  chart.prog = null;                     // the lens itself is global
  ...
  if (view === 'map' && s.lat && map && !phoneSheet()) {
    clearTimeout(openSide.pan);
    const reveal = () => {
      const mc = map.getContainer();
      if (current !== s || !mc.clientWidth) return;
      ...
      map.panInside([s.lat, s.lon], { paddingTopLeft: [left, 24], paddingBottomRight: [24, 24], animate: !prefersStill() });
    };
    openSide.pan = setTimeout(() => (map._flyToFrame ? map.once('moveend', reveal) : reveal()), prefersStill() ? 0 : 260);
  }
}
function clearScope() {
  chart.prog = null;
...
let sideReturn = null;     // a wish or the search button, for closing to return to
let sideClosing = false;   // a ✕ in flight: the next popstate is a CLOSE,
                           // whatever school the entry underneath names
function closeSide(fromHistory) {
  ...
  const openedName = sideOpener && current ? current.name : null;
  ...
  sideTrap(false);
  current = null;      // or the next county switch writes the closed school back into the URL
  setUrlSchool(null);
  liftMapControls();                       // the legend is visible again
  side.setAttribute('inert', '');          // keep 17 hidden controls out of Tab
  const opener = openedName
  ...
  sideOpener = null;
  const ret = sideReturn && (sideReturn.id ? document.getElementById(sideReturn.id)
    : [...document.querySelectorAll('#choices .who')].find(b => b.getAttribute('aria-label') === sideReturn.who));
  sideReturn = null;
  ...
  const fallback = [ret, 'map-fylke', 'map-cat', 'panel-sum', 'searchov-btn']
    .map(e => typeof e === 'string' ? document.getElementById(e) : e).find(e => e && e.offsetParent)
  ...
  (row || opener || fallback)?.focus({ preventScroll: view === 'list' && (!row || (rr.top >= 0 && rr.bottom <= innerHeight)) });
...
const photoSrc = u => (/\/bv\.ashx\//.test(u) && location.protocol === 'https:'
...
function renderSide() {
  ...
  if (!current || !document.getElementById('side').classList.contains('open')) return;
  const s = current;
  // photo header
  ...
  if (miniMapRO) { miniMapRO.disconnect(); miniMapRO = null; }
  ...
  const cy = (DATA.counties || []).find(c => c.fylke === s.fylke) || {};
  ...
  const { latest, prev, mean, meanPrev } = step;
...
let chanceMoreOpen = false;
const chanceMore = inner =>
  `<details class="more"${chanceMoreOpen ? ' open' : ''} ontoggle="chanceMoreOpen = this.open">` +
...
// «alle programområder»
function capFirst(x) { return x.charAt(0).toUpperCase() + x.slice(1); }
function renderChance(s, lensCat) {
  ...
  const ch = schoolChance(s, lensCat || 'all', myPoints);
  if (!ch) { box.innerHTML = `<div>${esc(t('chanceNoneInScope'))}</div>` + (adj ? chanceMore(adj) : ''); return; }
  const noPred = Math.max(0, ch.total - ch.n);
  const head = ch.likely ? t('chanceHeadL', fmt(myPoints), ch.likely, ch.n, noPred, ch.total)
             : ch.possible ? t('chanceHeadR', fmt(myPoints), ch.possible, ch.n, noPred, ch.total)
             : t('chanceHeadU', fmt(myPoints), ch.n, noPred, ch.total);
  ...
  const sf = Object.values((MODEL.meta || {}).sigma_forecast || {});
  const sm = Object.values((MODEL.meta || {}).sigma_level_multiplier || {});
  ...
  const cov = MODEL.meta && MODEL.meta.backtest_eval_years && MODEL.meta.backtest_eval_years.coverage80;
  ...
  if (fb) {
    ...
    let L3 = 0;
    for (const p of ch.progs) if (bucketOf(chanceFinal(predFor(s, p), myPoints, p.category, fb)) === 'likely') L3++;
...
function seriesFor(mode, cat, prog) {
  const progs = mode === 'prog' ? [prog]
    : mode === 'cat' ? shownPrograms(current).filter(p => p.category === cat)
    : shownPrograms(current);
...
function renderChartCard() {
  ...
  tabs.forEach(b => {
    ...
    b.onclick = () => {
      if (b.dataset.mode === 'all') {
        ...
        const present = [...new Set(shownPrograms(current).map(p => p.category))];
        setLens(mapCat !== 'all' && present.includes(mapCat) ? mapCat : present[0]);
      } else {
        const pool = levelScope(current.programs).filter(p => mapCat === 'all' || p.category === mapCat);
        chart.prog = pool.find(p => numericLatest(p.values)) || pool[0] || null;
  ...
  if (chartMode() === 'cat') {
    const present = [...new Set(levelScope(current.programs).map(p => p.category))];
    if (!present.includes(mapCat)) present.push(mapCat);   // lens the school lacks
    cs.hidden = false;
...
function drawChart() {
  ...
  const own = [...new Set(shownPrograms(current).flatMap(p => Object.keys(p.values)))].sort();
  const allYears = DATA.years.map(String);
  const years = own.length > 1
    ? allYears.filter(y => y >= own[0] && y <= own[own.length - 1])
    : own;
  const W = 444, H = 190, ML = 30, MR = 12, MT = 16, MB = 22;
  const iw = W - ML - MR, ih = H - MT - MB;
  const series = seriesFor(chartMode(), mapCat, chart.prog);
  ...
  if (!series.length) {
    ...
    const offered = mapCat === 'all' || shownPrograms(current).some(p => p.category === mapCat);
    document.getElementById('chart-sub').textContent = offered ? t('chartNoPoints') : `${CATS[mapCat][lang]} · ${t('notOffered')}`;
    svgHost.innerHTML = '';
    return;
  }
  ...
  const chartScope = chartMode() === 'cat'
    ? shownPrograms(current).filter(p => p.category === mapCat) : shownPrograms(current);
  ...
  sub.textContent = chartMode() === 'prog'
    ? `${chart.prog ? progName(chart.prog) + ' · ' : ''}${t('chartSubProg')}`
...
function renderList() {
  ...
  const scope = mapCat !== 'all' ? mapCat : null;
  const base = shownPrograms(current);
  ...
  for (const c of Object.keys(CATS).filter(c => byCat[c])) {
    ...
    const solo = list.length === 1 && progName(list[0]).toLowerCase() === CATS[c][lang].toLowerCase();
    if (!solo) html += `<button class="cat-head" data-cat="${c}" aria-pressed="${mapCat === c}"><span>${CATS[c][lang]}</span><span class="cnt">${list.length}</span></button>`;
    for (const p of list) {
      ...
      const sel = chart.prog === p ? ' sel' : '';
      let chip = '', chipTxt = '';
      if (chanceMode() && !orphan) {
        const pr = predFor(current, p);
        if (pr) {
          const ch = chanceOf(pr, myPoints);
          let tip = t('chTitle', pct(ch), pr.year, fmt(pr.m), fmt(pr.s), pct(pr.pi), pr.h);
          const fb = finalRoundBridge(current);
          if (fb) tip += ' ' + t('finalRoundChip', fb.to_round, pct(chanceFinal(pr, myPoints, p.category, fb)));
        ...
        } else if ((modelEntry(current, p) || {}).h === 0) {
          chip = `<span class="ch none" data-tip="${esc(t('noHistTitle'))}">${t('noHist')}</span>`;
          chipTxt = t('noHistTitle');
        }
      }
      const on = isChosen(current, p), idx = current.programs.indexOf(p);
  ...
  const inLevel = levelScope(current.programs);
  const hiddenMatch = showOld ? []
    : inLevel.filter(p => !base.includes(p) && (!scope || p.category === scope));
  const anyOld = inLevel.some(p => !isRecent(p, current.fylke));
  let oldBtn = '';
  if (hiddenMatch.length) oldBtn = `<button class="oldnote">${esc(t('oldHidden', hiddenMatch.length))}</button>`;
  else if (showOld && anyOld) oldBtn = `<button class="oldnote">${esc(t('oldShown'))}</button>`;
  ...
  const later = current.programs.filter(p => !inLevel.includes(p) && (!scope || p.category === scope)
                                             && (showOld || isRecent(p, current.fylke)));
  const anyLater = current.programs.some(p => !isVg1(p));
  let lvBtn = '';
  ...
  else if (allLevels && anyLater) lvBtn = `<button class="lvnote">${esc(t('levelsShown'))}</button>`;
  ...
  el.querySelector('.oldnote')?.addEventListener('click', () => {
    showOld = !showOld;
    try { localStorage.setItem('pk-showold', showOld ? '1' : '0'); } catch (e) {}
  ...
  el.querySelectorAll('.cat-head[data-cat]').forEach(b => b.onclick = () => {
    const cat = b.dataset.cat;
    chart.prog = null; setLens(cat);
  ...
  el.querySelectorAll('.prow').forEach(r => r.onclick = ev => {
    if (ev.target.closest('.pick')) return;
    chart.prog = current.programs[+r.dataset.idx]; renderChartCard(); renderList();
    refocus(`#s-list .prow[data-idx="${r.dataset.idx}"] button.nm`);
  });
  el.querySelectorAll('.pick').forEach(k => k.onclick = ev => {
    ev.stopPropagation();
    toggleChoice(current, current.programs[+k.dataset.idx]);
    refocus(`#s-list .pick[data-idx="${k.dataset.idx}"]`);
  });
  bindTips(el);
  // what a chip or the level says on hover, on keyboard focus too
  el.querySelectorAll('.prow button.nm').forEach(b => {
...
let contactOpener = null;   // the control that opened the sheet; focus returns to it
let bugCtx = null;          // the snapshot, taken when a bug button was pressed
let DATA_STAMP = '';        // schools.json's Last-Modified, when the server sends one
// child nodes joined with a space: «49,1<small>2026</small>» reads 49,1 2026
const text1 = el => [...el.childNodes].map(n => n.textContent.trim()).filter(Boolean).join(' ').replace(/\s+/g, ' ');
function bugContext(school, from) {
  const lv = [...document.body.classList].find(c => /^lv-(wide|split|thin)$/.test(c));
  const ctx = {
    from, link: location.href,
    view: view + (lv ? ` (${lv.slice(3)})` : ''),
    fylke: mapFylke, program: mapCat, points: myPoints == null ? '' : String(myPoints),
    levels: allLevels ? 'Vg1–Vg3' : 'Vg1', history: showOld ? 'on' : 'off',
    choices: String(choices.length), lang, font: PREFS.font, theme: PREFS.theme,
    cvd: PREFS.cvd ? 'on' : 'off',
    viewport: `${innerWidth}×${innerHeight} @${devicePixelRatio}`,
    ua: navigator.userAgent,
    data: `${DATA.schools.length} skoler, ${DATA.years[0]}–${DATA.years[DATA.years.length - 1]}`
          + (DATA_STAMP ? `, ${DATA_STAMP}` : ''),
  };
  if (!school && document.getElementById('side').classList.contains('open')) school = current;
  if (school) {
    ctx.school = `${school.name} (${school.fylke})`;
    ctx.chart = chartMode() + (chart.prog ? `: ${chart.prog}` : '');
...
function renderContactCtx() {
  if (!bugCtx) return '';
  const items = Object.entries(bugCtx).filter(([, v]) => v !== '' && !(Array.isArray(v) && !v.length))
...
function openBug(school, el) {
  bugCtx = bugContext(school || null, school ? 'school' : 'header');
  contactOpener = el || document.activeElement;
  openContact('feil');
}

function contactField(name) {
  ...
  const prev = document.getElementById(`c-${name}`);
  const value = prev ? prev.value
    : name === 'school' && open && current ? current.name
    : name === 'fylke' && mapFylke !== 'all' ? mapFylke : '';
  const type = name === 'photo' ? 'url' : name === 'year' ? 'number' : 'text';
  const extra = name === 'year'
    ? ` min="${DATA.years[0]}" max="${DATA.years[DATA.years.length - 1]}"` : '';
...
function renderContactFields() {
  ...
  if (kind === 'feil' && !bugCtx) bugCtx = bugContext(null, 'form');
...
function renderContact() {
  ...
  if (keep['c-kind']) document.getElementById('c-kind').value = keep['c-kind'];
...
async function sendContact(ev) {
  ...
  const mail = document.getElementById('c-email');
  ...
  const payload = {
    type: document.getElementById('c-kind').value,
    message: msg.value,
    email: document.getElementById('c-email').value,
    website: document.getElementById('c-website').value,
    lang, page: location.pathname + location.search + location.hash,
  };
  document.querySelectorAll('#c-extra input').forEach(i => { payload[i.dataset.f] = i.value; });
  if (payload.type === 'feil' && bugCtx) payload.context = bugCtx;
  btn.disabled = true;
...
function openContact(kind) {
  ...
  renderContact();
  if (kind) { document.getElementById('c-kind').value = kind; renderContactFields(); }
  showSheet('contact');
  setModalTrap();
  openSheetHistory();
...
function closeContact(fromHistory) {
  if (!fromHistory && (history.state || {}).pkSheet) { history.back(); return; }
  hideSheet('contact');
  setModalTrap();
  const back = contactOpener && document.contains(contactOpener) ? contactOpener
             : document.getElementById('help-btn');
  contactOpener = null; bugCtx = null;
  back?.focus();
}
...
function bindTips(root) {
  const attach = (el, html) => {
    ...
    el.addEventListener('touchstart', ev => {
      ...
      showTip.timer = setTimeout(hideTip, 4500);
    }, { passive: false });
  };
...
function renderIntro() {
  ...
  document.getElementById('intro-body').innerHTML =
    `<p class="lede">${esc(t('introLede'))} ${esc(t('introScope', DATA.schools.length, DATA.counties.length,
      DATA.years[0], DATA.years[DATA.years.length - 1]))}</p>${steps}` +
...
const SHEET_IDS = ['contact', 'intro', 'settings', 'calc', 'searchov'];
...
const isSheetOpen = el => el && !el.hidden && !el.classList.contains('closing');
const anySheetOpen = () => SHEET_IDS.some(id => isSheetOpen(document.getElementById(id)));
...
const SHEETS = ['searchov', 'settings', 'intro', 'calc', 'contact'];
document.addEventListener('keydown', e => {
  ...
  if (!f.length) return;
  const first = f[0], last = f[f.length - 1], outside = !sheet.contains(e.target);
...
function setModalTrap() {
  const open = anySheetOpen();
  ['app', 'legend'].forEach(id => document.getElementById(id)?.toggleAttribute('inert', open));
}

let sheetPushPending = false;   // openSheetHistory() called while closeSide's back() is in flight
function openSheetHistory() {
  ...
  if (sideClosing) { sheetPushPending = true; return; }
...
function closeIntro(fromHistory) {
  if (!fromHistory && (history.state || {}).pkSheet) { history.back(); return; }
  hideSheet('intro');
  try { localStorage.setItem(INTRO_SEEN, '1'); } catch (e) {}
  setModalTrap();
...
const PREFS = { theme: 'auto', font: 'n', cvd: false };
function loadPrefs() {
  try {
...
function renderSettings() {
  ...
  document.getElementById('settings-body').innerHTML =
    ...
    seg('levels', [['1', 'Vg1'], ['all', esc(t('levelsChipAll'))]], allLevels ? 'all' : '1', t('levelsSumLabel')) +
    `<div class="hint">${esc(t('setLevelsHint'))}</div></div>` +
    `<div class="row"><div class="t">${esc(t('langLabel'))}</div>` +
    seg('lang', [['no', 'NO'], ['en', 'EN']], lang, t('langLabel')) + `</div>` +
  ...
  document.getElementById('settings-body').querySelectorAll('.seg button').forEach(b => {
    b.onclick = () => {
      ...
      document.querySelector(`#settings .seg button[data-k="${k}"][data-v="${v}"]`)?.focus();
    };
  });
}
function openSettings() {
  ...
  setModalTrap();
  openSheetHistory();
  setTimeout(() => document.querySelector('#settings .seg button.on')?.focus(), 40);
}
...
const foldName = s => s.toLowerCase()
  .replace(/æ/g, 'ae').replace(/ø/g, 'o').replace(/å/g, 'a').replace(/é/g, 'e')
  .replace(/[.,-]/g, ' ')
  .replace(/\s+/g, ' ').trim();
let searchIx = null;
function runSearch(q) {
  if (!DATA) return null;                      // a keystroke can beat the fetch
  if (!searchIx) searchIx = DATA.schools.map(s => ({ s, f: foldName(s.name) }));
  const f = foldName(q);
  if (!f) return null;
...
function renderOvList() {
  ...
  box.innerHTML = !q.value.trim() ? ''
    ...
    ? ovHits.map((s, i) =>
        s.county
        ? `<div class="opt county${i === ovAct ? ' act' : ''}" id="ov-opt-${i}" role="option"` +
          ` aria-selected="${i === ovAct}"><span>${esc(t('searchCounty', s.county))}</span></div>`
        : `<div class="opt${i === ovAct ? ' act' : ''}" id="ov-opt-${i}" role="option"` +
        ` aria-selected="${i === ovAct}"><span>${esc(s.name)}</span>` +
  ...
  if (ovAct >= 0) q.setAttribute('aria-activedescendant', 'ov-opt-' + ovAct);
  ...
  if (ovAct >= 0) document.getElementById('ov-opt-' + ovAct)?.scrollIntoView({ block: 'nearest' });
}
function pickOv(i) {
  const s = ovHits[i];
  if (!s) return;
  ovHits = []; ovAct = -1;               // a second event on the same option is a no-op
  ...
  if (s.county) {                          // the county row: filter, as the select would
    const sel = document.getElementById('map-fylke');
  ...
  if (s.lat && view === 'map') {
    ...
    prefersStill() ? map.setView([s.lat, s.lon], z) : map.flyTo([s.lat, s.lon], z);
  }
  openSide(s);
}
function openSearchOv() {
  const q = document.getElementById('ov-q');
  q.value = ''; ovHits = []; ovAct = -1;
...
function closeSearchOv(fromHistory) {
  if (!fromHistory && (history.state || {}).pkSheet) { history.back(); return; }
  hideSheet('searchov');
  setModalTrap();
...
let locLayer = null, locBtnEl = null, locBusy = false;
...
function toast(msg, ms = 4000) {
  ...
  if (!el._tap) {
    el._tap = true;
    el.addEventListener('click', () => { clearTimeout(toast._t); el.hidden = true; });
  }
...
function addLocateControl() {
  ...
  new C({ position: 'bottomright' }).addTo(map);
  updateLocateAria();
}
// Leaflet's own zoom buttons are titled in English; keep them in the app's language
function updateZoomAria() {
  [['.leaflet-control-zoom-in', 'zoomIn'], ['.leaflet-control-zoom-out', 'zoomOut']].forEach(([sel, key]) => {
    document.querySelectorAll(sel).forEach(a => { a.title = t(key); a.setAttribute('aria-label', t(key)); });
  });
}
function updateLocateAria() {
  if (!locBtnEl) return;
  locBtnEl.title = t('locBtn');
  locBtnEl.setAttribute('aria-label', t('locBtn'));
  locBtnEl.setAttribute('aria-pressed', String(!!locLayer));
}
function locate() {
  if (locBusy) return;                  // one fix in flight at a time
  if (locLayer) {                       // second press clears the marker again
    map.removeLayer(locLayer); locLayer = null;
    locBtnEl.classList.remove('on');
    updateLocateAria();
    return;
  }
  locBusy = true;
  navigator.geolocation.getCurrentPosition(pos => {
    ...
    locBtnEl.classList.add('on');
    updateLocateAria();
    const z = Math.max(map.getZoom(), 10);
    prefersStill() ? map.setView(ll, z) : map.flyTo(ll, z);
  }, err => {
    locBusy = false;
...
/* ================= list view ================= */
let view = 'map';
let listSort = { key: 'value', dir: -1 };
let refitPending = false;      // a county switch made while the map was hidden
function setView(v) {
  if (v !== 'map' && v !== 'list') return;
  if (v === 'list' && !DATA) return;     // the toggle is live before the fetch lands
  view = v;
  ...
  if (v === 'list') renderListView();
    ...
    if (refitPending) {
      refitPending = false;
      const pts = visibleSchools().filter(s => s.lat).map(s => [s.lat, s.lon]);
      if (pts.length) map.fitBounds(L.latLngBounds(pts).pad(0.08), { ...framePad(), animate: false });
    }
  }
}
...
function sortList(key) {
  ...
  listSort = listSort.key === key
    ? { key, dir: -listSort.dir }
    : { key, dir: key === 'name' || key === 'fylke' ? 1 : -1 };
  renderListView();
  // the table was rebuilt under the pressed header; keep the reader on it
  if (fromHeader) document.querySelector(`#listview th.col-${key} button`)?.focus({ preventScroll: true });
}
function renderListView() {
  const host = document.getElementById('listview');
  const chance = chanceMode();
  const allF = mapFylke === 'all';
  // a sort key can outlive its column (points cleared, county picked)
  if ((listSort.key === 'chance' && !chance) || (listSort.key === 'fylke' && !allF)) {
    listSort = { key: 'value', dir: -1 };
  }
  const rows = visibleSchools().map(s => {
    const pr = schoolPressure(s, mapCat);
    const v = pr.kind === 'points' ? pr.v : null;
    const sc = chance ? schoolChance(s, mapCat, myPoints) : null;
    return { s, pr, v, delta: pr.kind === 'points' ? deltaFor(s, mapCat, pr.year) : null,
             sc, best: sc ? sc.best : null };
  });
  const nil = x => x === null || x === undefined;
  rows.sort((a, b) => {
    const k = listSort.key;
    if (k === 'name') return a.s.name.localeCompare(b.s.name, 'no') * listSort.dir;
    if (k === 'fylke') return (a.s.fylke.localeCompare(b.s.fylke, 'no')
      || a.s.name.localeCompare(b.s.name, 'no')) * listSort.dir;
    ...
    if (nil(va)) return 1;
    if (nil(vb)) return -1;
    return (va - vb) * listSort.dir || a.s.name.localeCompare(b.s.name, 'no');
  });
  ...
  const arrow = k => `<span aria-hidden="true" class="ar${listSort.key === k ? ' on' : ''}">` +
    `${listSort.key === k && listSort.dir > 0 ? '↑' : '↓'}</span>`;
  const ariaSort = k => listSort.key === k
    ? ` aria-sort="${listSort.dir < 0 ? 'descending' : 'ascending'}"` : '';
  ...
  const th = (k, label, num, tip) =>
    `<th class="col-${k} ${num ? 'num ' : ''}${listSort.key === k ? 'on' : ''}"${ariaSort(k)} scope="col"` +
    `${tip ? ` title="${esc(tip)}"` : ''}>` +
  ...
  const catLabel = mapCat === 'all' ? t('allCats') : (CATS[mapCat] || {})[lang] || mapCat;
  const fyLabel = allF ? t('allFylker') : mapFylke;
  const chip = r => {
  ...
  host.innerHTML =
    ...
    `<table aria-labelledby="list-title" class="${chance ? 'has-chance' : ''}${listSort.key === 'delta' ? ' by-delta' : ''}"><thead><tr>` +
    ...
    (chance ? th('chance', t('listColChance'), false, t('listColChanceTip', fmt(myPoints), forecastYears(rows.map(r => r.s)))) : '') +
    `</tr></thead><tbody>` +
    rows.map((r, i) =>
      `<tr data-i="${i}">` +
...
const CALC_EXAMS = ['Skriftlig eksamen', 'Muntlig eksamen'];
let calcGrades = {};
function loadCalc() {
  try {
...
function saveCalc() {
  try {
    Object.keys(calcGrades).length
      ? localStorage.setItem('pk-grades', JSON.stringify(calcGrades))
      : localStorage.removeItem('pk-grades');
  } catch (e) {}
}
function calcMean() {
  const vs = Object.values(calcGrades);
...
function renderCalc() {
  ...
  const mean = calcMean();
  const fmtN = v => v.toFixed(2).replace('.', lang === 'no' ? ',' : '.');
  const fmtP = v => v.toFixed(1).replace('.', lang === 'no' ? ',' : '.');
  document.getElementById('calc-body').innerHTML =
    ...
    ` <button class="cta ghost" id="calc-reset" ${Object.keys(calcGrades).length ? '' : 'hidden'}>${esc(t('calcReset'))}</button>` +
    `<p class="note">${esc(t('calcNote'))}</p>`;
  document.getElementById('calc-body').querySelectorAll('.grades button').forEach(b => {
    b.onclick = () => {
      const f = b.dataset.f, g = b.dataset.g;
      if (!g) delete calcGrades[f];
      else calcGrades[f] = +g;
      saveCalc(); renderCalc();
      ...
      document.querySelector(`#calc .grades button[data-f="${CSS.escape(f)}"][data-g="${g || '1'}"]`)?.focus();
    };
  });
  ...
  if (reset) reset.onclick = () => {
    calcGrades = {}; saveCalc(); renderCalc();
    document.querySelector('#calc .grades button')?.focus();
  };
}
function openCalc() {
  renderCalc();
  ...
  setModalTrap();
  openSheetHistory();
  setTimeout(() => document.querySelector('#calc .grades button')?.focus(), 40);
}
function closeCalc(fromHistory) {
  if (!fromHistory && (history.state || {}).pkSheet) { history.back(); return; }
  hideSheet('calc');
  setModalTrap();
...
async function main() {
  ...
  try {
    ...
    if (l === 'no' || l === 'en') lang = l;   // anything else is not a language
  } catch (e) {}
  try { showOld = localStorage.getItem('pk-showold') === '1'; } catch (e) {}
  try { allLevels = localStorage.getItem('pk-alllevels') === '1'; } catch (e) {}
  ...
  try {
    ...
    if (!r.ok) throw new Error(String(r.status));
    DATA = await r.json();
    try { DATA_STAMP = r.headers.get('last-modified') || ''; } catch (e) {}
  ...
  try {
    ...
    if (rm && rm.ok) MODEL = await rm.json();
  ...
  try {
    ...
    if (MODEL && sp.pts !== null) {
      myPoints = sp.pts;
      document.getElementById('my-points').value = String(sp.pts).replace('.', lang === 'no' ? ',' : '.');
    } else if (sp.pts === null) {
  ...
  HOME = L.latLngBounds(pts).pad(0.06);
  ...
  map = L.map('map', { zoomControl: false, zoomSnap: 0.25,
    ...
    ...(prefersStill() ? { zoomAnimation: false, fadeAnimation: false, markerZoomAnimation: false, inertia: false } : {}) }).fitBounds(HOME, framePad());
  L.control.zoom({ position: 'bottomright' }).addTo(map);
  updateZoomAria(); updateMapLabels();
  addLocateControl();
  map.on('zoomend moveend', () => { labelMarkers(); legendZoomHint(); });
  setTiles();
  let touched = false;
  ['mousedown', 'wheel', 'touchstart'].forEach(ev => map.getContainer()
  ...
  function ensureFramed() {
    ...
    if (view === 'list') { refitPending = true; return true; }   // reframed on the way back
    if (!map.getContainer().clientWidth) return false;            // still nothing to frame
    map.invalidateSize();
    map.fitBounds(HOME, { ...framePad(), animate: false });
    framed = true;
    return true;
  }
  ...
  if (window.ResizeObserver) new ResizeObserver(() => {
    if (map.getContainer().clientWidth) {
      map.invalidateSize({ pan: innerWidth !== lastW });
      lastW = innerWidth;
      if (view === 'map') liftMapControls();
    }
    ensureFramed();
  ...
  const ovq = document.getElementById('ov-q');
  ovq.addEventListener('input', () => {
    ovAct = -1; ovHits = runSearch(ovq.value) || []; renderOvList();
  });
  ovq.addEventListener('keydown', ev => {
    if (ev.key === 'ArrowDown') { ev.preventDefault(); ovAct = Math.min(ovAct + 1, ovHits.length - 1); renderOvList(); }
    else if (ev.key === 'ArrowUp') { ev.preventDefault(); ovAct = Math.max(ovAct - 1, -1); renderOvList(); }
    else if (ev.key === 'Enter') { ev.preventDefault(); pickOv(ovAct >= 0 ? ovAct : 0); }
  });
  document.addEventListener('keydown', ev => {
  ...
  addEventListener('resize', () => {
    ...
    reframe = setTimeout(() => {
      map.invalidateSize(); liftMapControls(); keepPanelFocusInView();
  ...
  document.addEventListener('touchstart', ev => {
    ...
    if (!ev.target.closest('[title], .ch, .lv, #tip')) hideTip();
  }, { passive: true });
  ...
  addEventListener('hashchange', () => {
    ...
    if (s && s !== current) {
      if (s.lat && view === 'map') map.setView([s.lat, s.lon], Math.max(map.getZoom(), 11));
      openSide(s);
  ...
  addEventListener('popstate', () => {
    ...
    if (sideClosing) {
      ...
      if (sheetPushPending) { sheetPushPending = false; openSheetHistory(); }
      return;
    }
    if (anySheetOpen()) {
    ...
    if (target && target !== current) { openSide(target); return; }
  ...
  setView(storedView);
  if (framedByUrl && view === 'map') {
    const pts = visibleSchools().filter(s => s.lat).map(s => [s.lat, s.lon]);
    if (pts.length) { touched = true; map.fitBounds(L.latLngBounds(pts).pad(0.08), { ...framePad(), animate: false }); }
  }
  ...
  if (linked) {
    if (linked.lat) {
      ...
      map.setView([linked.lat, linked.lon], 11, { animate: false });
    }
    openSide(linked);
  }
...
const boot = () => main().catch(e => { console.error(e); bootFailed('bootFailApp'); });
