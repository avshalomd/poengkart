...
export const T = {
  ...
  introSteps: {
    no: [
      ['Poengsummen din',
       'Poengsum = karaktersnittet ditt × 10. Poenggrensen er poengsummen til den siste søkeren som fikk plass – lå du over den, ville du kommet inn det året. Sjansen for neste inntak regnes mot poenggrensen prognosen venter, som kan ligge over eller under fjorårets.'],
      ...
      ['Sjansen din',
       'Skriv inn poengene dine, så farges kartet etter sjansen for plass: grønt = sannsynlig, gult = mulig, rødt = lite sannsynlig. Under hver sjanse står poenggrensen prognosen venter. Trykk + på et programområde for å samle ønskene dine og se om lista holder. Regnet ut fra historikken.'],
    ],
    en: [
      ['Your points',
       'Points = your grade average × 10. The threshold is the score of the last applicant who got a place – if you were above it, you would have got in that year. Your chance at the next intake is measured against the threshold the forecast expects, which can sit above or below last year’s.'],
      ['Colours and sizes',
      ...
      ['Your chance',
       'Enter your points and the map recolours by your chance of a place: green = likely, amber = possible, red = unlikely. Under each chance is the threshold the forecast expects. Press + on a programme area to collect your wishes and see whether the list holds up. Worked out from the history.'],
    ],
  },
  ...
  introWordsH: { no: 'Ord du møter', en: 'Words you will meet' },
  introWords: {
    no: [
      ['Poenggrense', 'Poengsummen til den siste som fikk plass. Har skolen flere programområder, viser toppen av skolesiden snittet av grensene – radene under viser tallet som gjelder deg.'],
      ['1., 2. og 3. inntak', 'Tilbudene om skoleplass kommer i runder utover sommeren. Grensen faller ofte fra ett inntak til det neste, fordi plasser blir ledige. Merket på skolen viser hvilket inntak tallene er fra.'],
      ['Programområde', 'Det du søker på innenfor et utdanningsprogram, for eksempel Musikk eller Dans. Hvert programområde har sin egen poenggrense.'],
      ['SK 3 år', 'Et yrkesfaglig løp som gir studiekompetanse etter tre år. Det har sin egen poenggrense.'],
      ['Ingen venteliste og Fullt', 'Ingen venteliste: alle kvalifiserte søkere fikk plass. Fullt, siste inntatte uten poeng: plassene ble fylt, men den siste som fikk plass, hadde ingen registrerte poeng.'],
      ['Inntaksområde', 'Noen fylker deler inn i inntaksområder, og søkere som bor i skolens område, går foran. Fylkets inntaksregler sier hvilket område du hører til.'],
    ],
    en: [
      ['Poenggrense (threshold)', 'The points of the last applicant admitted. Where a school has several programme areas, the top of the school sheet shows the average of their thresholds – the rows below show the figure that applies to you.'],
      ['1st, 2nd and 3rd intake', 'Offers of a place come in rounds over the summer. The threshold often falls from one intake to the next, as places free up. The chip on each school says which intake its figures are from.'],
      ['Programme area', 'What you apply for within an education programme, such as Music or Dance. Each programme area has its own threshold.'],
      ['SK 3 år', 'A vocational track that leads to general university admission in three years. It has its own threshold.'],
      ['No waiting list and Filled', 'No waiting list: every qualified applicant got a place. Filled, last admitted without points: the places filled, but the last applicant admitted had no registered points.'],
      ['Inntaksområde (catchment)', 'Some counties divide into catchment areas, and applicants living in the school’s area go first. The county’s admission rules say which area you belong to.'],
    ],
  },
  introApplyH: { no: 'Slik søker du', en: 'How to apply' },
  introApply: {
    no: 'Du søker på vigo.no innen 1. mars det året du går på 10. trinn, med ønskene i den rekkefølgen du helst vil ha dem. Rådgiveren på skolen din hjelper deg. Vilbli.no har reglene for fylket ditt.',
    en: 'You apply at vigo.no by 1 March of the year you are in 10. trinn, with your wishes in the order you prefer them. The rådgiver (counsellor) at your school will help. Vilbli.no has the rules for your county.',
  },
  introCta: { no: 'Vis kartet', en: 'Show the map' },
  helpLabel: { no: 'Slik leser du kartet', en: 'How to read this map' },
  langSwitch: { no: 'In English', en: 'På norsk' },
  ...
  listColVal:    { no: 'Grense', en: 'Threshold' },
  listColValTip: { no: 'Poenggrensen siste publiserte år. Har skolen flere programområder, er tallet snittet av grensene.',
                   en: 'The threshold in the latest published year. Where the school has several programme areas, the figure is the average of their thresholds.' },
  ...
  listChanceCell: { no: (b, p, n) => `${T.bandWord.no(b)} · ${p}${n > 1 ? `, best av ${n}` : ''}`,
                    en: (b, p, n) => `${T.bandWord.en(b)} · ${p}${n > 1 ? `, best of ${n}` : ''}` },
  bandWord: { no: b => ({ likely: 'Sannsynlig', possible: 'Mulig', unlikely: 'Lite sannsynlig' })[b],
              en: b => ({ likely: 'Likely', possible: 'Possible', unlikely: 'Unlikely' })[b] },
  ...
  calcLede:  { no: 'Karakterpoengene er gjennomsnittet av alle tallkarakterene på vitnemålet ganget med 10. Standpunkt og eksamen teller likt; fag uten karakter regnes ikke med. Går du på 10. trinn og har ikke standpunkt ennå, bruk karakterene fra siste halvår. Trykk på karakterene dine:',
               en: 'Your points are the average of every numeric grade on your vitnemål (leaving certificate), times ten. The subjects below are named as they are printed on it. Standpunkt (teacher-assessed) and eksamen (exam) grades count equally; subjects without a grade are left out. In 10. trinn without final grades yet, use your latest term grades. Tap your grades:' },
  calcExamHead: { no: 'Eksamen – hopp over hvis du ikke har hatt den ennå', en: 'Exams – skip these if you have not had them yet' },
  ...
  heroTypical:{ no: 'Poenggrense', en: 'Threshold' },
  heroMeanOf: { no: (lo, hi) => `Snitt av grensene ${lo}–${hi}`, en: (lo, hi) => `Average of thresholds ${lo}–${hi}` },
  ...
  // a figure of 6 or less is a grade average, not points: nobody scores under
  // 10 (a straight-1 average), and a reader who typed «4,5» got an all-red map
  ptsAvg: { no: (x, p) => `${x} ser ut som et karaktersnitt. Poeng = snitt × 10.`,
  ...
  choicesAnyTitle: {
    no: 'Regnet som om ønskene var uavhengige av hverandre. Det er de ikke helt – et år med hard konkurranse rammer flere av dem samtidig, og to programområder ved samme skole henger sammen – så tallet er litt for høyt, og det vises aldri over 95 %.',
    en: 'Computed as if the wishes were independent of each other. They are not quite – a hard year hits several of them at once, and two programme areas at one school go together – so the figure is a little too high, and it is never shown above 95%.',
  },
  ...
  // ----- places: «Nær …» (places.ts, listview.ts setNear) -----
  nearBtn:     { no: 'Nær et sted', en: 'Near a place' },
  nearChip:    { no: p => `Nær ${p}`, en: p => `Near ${p}` },
  nearClear:   { no: 'Slutt å sortere etter avstand', en: 'Stop sorting by distance' },
  nearLabel:   { no: 'Sorter skolene etter avstand fra et sted', en: 'Sort the schools by distance from a place' },
  nearPh:      { no: 'Kommune eller tettsted…', en: 'Municipality or town…' },
  nearMe:      { no: 'Der jeg er nå (bruk posisjonen min)', en: 'Where I am now (use my location)' },
  nearMeName:  { no: 'deg', en: 'you' },
  nearPlace:   { no: p => `Nær ${p} – sorter etter avstand`, en: p => `Near ${p} – sort by distance` },
  nearSchools: { no: n => `${n} ${n === 1 ? 'skole' : 'skoler'}`, en: n => `${n} ${n === 1 ? 'school' : 'schools'}` },
  nearNone:    { no: 'Ingen skole ligger i et sted med det navnet. Prøv kommunen, eller bruk posisjonen din.',
                 en: 'No school stands in a place by that name. Try the municipality, or use your location.' },
  nearSet:     { no: p => `Skolene er sortert etter avstand fra ${p}.`, en: p => `The schools are sorted by distance from ${p}.` },
  nearCleared: { no: 'Skolene er ikke lenger sortert etter avstand.', en: 'The schools are no longer sorted by distance.' },
  nearFoot:    { no: 'Avstand i luftlinje, ikke reisevei. Stedet du valgte, lagres bare i denne nettleseren.',
                 en: 'Distance as the crow flies, not travel time. The place you picked is kept in this browser only.' },
  // ----- the List view's two row kinds -----
  listModeLabel:   { no: 'Vis én rad per', en: 'Show one row per' },
  listModeSchools: { no: 'Skole', en: 'School' },
  listModeAreas:   { no: 'Programområde', en: 'Programme area' },
  listCountAreas:  { no: n => `${n} ${n === 1 ? 'programområde' : 'programområder'} · siste publiserte år`,
                     en: n => `${n} programme area${n === 1 ? '' : 's'} · latest published year` },
  listAreasNoPts:  { no: 'Skriv inn poengene dine for å se sjansen for plass på hver rad.',
                     en: 'Enter your points to see the chance of a place on every row.' },
  listColPlace: { no: 'Sted', en: 'Place' },
  listColDist:  { no: 'Avstand', en: 'Distance' },
  listColDistTip: { no: p => `Avstand i luftlinje fra ${p}`, en: p => `Straight-line distance from ${p}` },
  listColCut:   { no: 'Grense', en: 'Threshold' },
  ...
  // ----- the wish list: order, copy, the capped total -----
  choicesAnyOver: { no: p => `Sjansen for minst én plass: over ${p} %`, en: p => `Chance of at least one place: over ${p}%` },
  choicesUpOne:   { no: (sch, what, r) => `Flytt ønske ${r}, ${what} ved ${sch}, opp`,
                    en: (sch, what, r) => `Move wish ${r}, ${what} at ${sch}, up` },
  choicesDownOne: { no: (sch, what, r) => `Flytt ønske ${r}, ${what} ved ${sch}, ned`,
                    en: (sch, what, r) => `Move wish ${r}, ${what} at ${sch}, down` },
  choicesCopy:    { no: 'Kopier liste', en: 'Copy list' },
  choicesCopied:  { no: 'Lista er kopiert. Lim den inn der du vil.', en: 'The list is copied. Paste it wherever you like.' },
  choicesCopyFail: { no: 'Nettleseren ga ikke tilgang til utklippstavlen. Marker lista og kopier den selv.',
                     en: 'The browser did not let us reach the clipboard. Select the list and copy it yourself.' },
  copyHead:    { no: 'Ønskene mine', en: 'My wishes' },
  copyHeadPts: { no: x => `Ønskene mine, med ${x} poeng`, en: x => `My wishes, with ${x} points` },
  copyFoot:    { no: (h, d) => `Fra ${h}, ${d}. Sjansene er anslag ut fra tidligere år, ikke en garanti.`,
                 en: (h, d) => `From ${h}, ${d}. The chances are estimates from earlier years, not a guarantee.` },
  band_likely:   { no: 'sannsynlig', en: 'likely' },
  band_possible: { no: 'mulig', en: 'possible' },
  band_unlikely: { no: 'lite sannsynlig', en: 'unlikely' },
};
export const t = (k, ...a) => {
