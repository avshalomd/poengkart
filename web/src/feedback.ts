...
export function bugContext(school, from) {
  ...
  const ctx: any = {
    ...
    levels: S.allLevels ? 'Vg1–Vg3' : 'Vg1', history: S.showOld ? 'on' : 'off', stale: S.showStale ? 'on' : 'off',
...
export const CTX_LABELS: Record<string, { no: string; en: string }> = {
  ...
  history: { no: 'Historikk', en: 'History' }, stale: { no: 'Skoler uten nye tall', en: 'Schools without recent figures' },
  choices: { no: 'Ønsker', en: 'Wishes' },
